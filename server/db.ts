...
export interface VolunteerProfile {
  ...
  rating_avg?: number;
  rating_count?: number;
  podium_permanent_medals?: string[];
  podium_history_days?: number;
  created_at: string;
  updated_at: string;
}
...
class Database {
  ...
  // --- Gallery System (Bloque E3 - Instagram/TikTok Social Features) ---
  public getGalleryItems(): GalleryItem[] {
    if (this.data.gallery.length === 0) {
      this.seedInitialGallery();
    }
  ...
  public createGalleryItem(item: GalleryItem): GalleryItem {
    ...
    this.data.gallery.unshift(enriched);
    this.save();
    return enriched;
  }

  public likeGalleryItem(id: string, userKey: string): { likes_count: number; is_liked: boolean } {
    const item = this.data.gallery.find(g => g.id === id);
    if (!item) {
      throw new Error('Elemento de galería no encontrado.');
    }

    item.liked_by = item.liked_by || [];
    item.likes_count = item.likes_count || 0;

    const existingIdx = item.liked_by.indexOf(userKey);
    let is_liked = false;

    if (existingIdx !== -1) {
      item.liked_by.splice(existingIdx, 1);
      item.likes_count = Math.max(0, item.likes_count - 1);
      is_liked = false;
    } else {
      item.liked_by.push(userKey);
      item.likes_count += 1;
      is_liked = true;
    }

    this.save();
    return { likes_count: item.likes_count, is_liked };
  }

  public addGalleryComment(id: string, commentData: { author_name: string; author_avatar?: string; comment: string }): GalleryComment {
    const item = this.data.gallery.find(g => g.id === id);
    if (!item) {
      throw new Error('Elemento de galería no encontrado.');
    }
    ...
    const newComment: GalleryComment = {
      id: `comm_${Date.now()}_${Math.floor(100 + Math.random() * 900)}`,
      author_name: commentData.author_name.trim() || 'Voluntario de la Comunidad',
  ...
  public shareGalleryItem(id: string): number {
    const item = this.data.gallery.find(g => g.id === id);
    if (!item) return 0;
    item.shares_count = (item.shares_count || 0) + 1;
    this.save();
    return item.shares_count;
  }
  ...
  private seedInitialGallery() {
    this.data.gallery = [
      {
        id: 'gal_001',
        title: 'Feria de Recursos BFL en East High School',
        description: 'Más de 350 familias recibieron mochilas, orientación bilingüe y libros con apoyo de 24 voluntarios dedicados.',
        image_url: 'https://images.unsplash.com/photo-1577896851231-70ef18881754?w=1000&auto=format&fit=crop&q=80',
        media_type: 'IMAGE',
        school_tag: 'East High School',
        event_title: 'Gran Feria de Recursos Escolares y Bienvenida a Familias',
        date: '2026-08-15',
        uploaded_by: 'Staff DMPS Connect',
        likes_count: 42,
        liked_by: [],
        comments: [
          {
            ...
            comment: '¡Fue una experiencia increíble apoyar a las familias de primer ingreso!',
            created_at: '2026-08-15T19:30:00Z',
          },
          {
            id: 'c2',
            author_name: 'Elena Rostova',
            comment: 'Orgullo total de nuestra comunidad escolar.',
      ...
      {
        id: 'gal_002',
        title: 'Jornada de Empaque y Despensa Solidaria',
        description: 'Voluntarios clasificando y armando cajas de víveres para el fin de semana.',
        image_url: 'https://images.unsplash.com/photo-1593113598332-cd288d649433?w=1000&auto=format&fit=crop&q=80',
        media_type: 'IMAGE',
        school_tag: 'Roosevelt High School',
        event_title: 'Jornada de Empaque y Distribución de Alimentos',
        date: '2026-08-12',
        uploaded_by: 'Jeferson Martinez (DMPS Connect)',
        likes_count: 38,
        liked_by: [],
        comments: [
          {
            id: 'c3',
            author_name: 'Mateo Gómez',
            comment: 'Excelente labor de equipo.',
      ...
      },
      {
        id: 'gal_003',
        title: 'Taller de Alfabetización Digital para Familias',
        description: 'Asistencia tecnológica personalizada a padres de familia para el uso de plataformas educativas.',
        image_url: 'https://images.unsplash.com/photo-1531482615713-2afd69097998?w=1000&auto=format&fit=crop&q=80',
        media_type: 'IMAGE',
        school_tag: 'Lincoln High School',
        event_title: 'Taller de Alfabetización Digital para Familias',
        date: '2026-08-08',
        uploaded_by: 'Coordinación BFL',
        likes_count: 56,
        liked_by: [],
        comments: [
          {
            ...
            comment: 'Ver a los padres aprender a usar el portal es muy gratificante.',
  ...
  // --- Real-Time Statistics for Home and Hub ---
  public getLiveStats(): {
    ...
    silver_cord_honorees_count: number;
    schools_impacted_count: number;
  } {
    const totalApprovedMin = this.data.hour_submissions
    ...
    this.data.profiles.forEach(p => {
      if (p.school) distinctSchools.add(p.school.trim());
    });
    this.data.hour_submissions.forEach(s => {
      if (s.school) distinctSchools.add(s.school.trim());
    });

    const openEvents = this.data.events.filter(e => e.status === 'OPEN' || e.status === 'FEW_SPOTS').length;
    const silverCordCount = this.getSilverCord160Honorees().length;

    return {
      total_approved_minutes: totalApprovedMin,
      total_approved_hours: parseFloat((totalApprovedMin / 60).toFixed(1)),
      total_volunteers: this.data.profiles.length,
      total_events: this.data.events.length,
      total_open_events: openEvents,
      total_submissions: this.data.hour_submissions.length,
      total_certificates_issued: this.data.certificates.length,
      silver_cord_honorees_count: silverCordCount,
      schools_impacted_count: Math.max(distinctSchools.size, 5),
    };
  }
