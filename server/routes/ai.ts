...
function fallbackExtractHours(rawText: string): ExtractedHourItem[] {
  ...
  for (let i = 0; i < rawChunks.length; i++) {
    ...
    let supervisor = 'Brenda Lucero/Veronica Ortiz';
  ...
  if (results.length === 0 && rawText.trim().length > 0) {
    results.push({
      ...
      supervisor_name: 'Brenda Lucero/Veronica Ortiz',
...
5. REGLAS ESPECÍFICAS DE PARSEO DMPS:
   - "Total Hours" y "Loc 1 Hours" / "Horas Ubicación 1" son el mismo valor; NO las sumes.
   - "Location 1" / "Ubicación 1" contiene el nombre del evento o la ubicación.
   - "Grade" / "Grado" se refiere al nivel de grado del estudiante al momento del evento (ej. 9°, 10°, 11°, 12°).
   - Si no se menciona un supervisor específico, asigna por defecto "Brenda Lucero/Veronica Ortiz".
