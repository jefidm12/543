...
export const api = {
  ...
  async adminEditSubmission(id: string, editData: Partial<HourSubmission> & { hours?: number; minutes?: number }) {
    const res = await fetch(`${API_BASE}/submissions/${id}/admin-edit`, {
      method: 'PUT',
      headers: {
        'Content-Type': 'application/json',
        ...getAuthHeader(),
    ...
    return handleResponse<{
      ...
      volunteer_stats?: { approved_minutes: number; pending_minutes: number };
    }>(res);
  },
  ...
  async getLiveCommunityStats() {
    const res = await fetch(`${API_BASE}/public/live-stats`);
    return handleResponse<{
      success: boolean;
      stats: {
  ...
  // Gallery API
  async getGallery(school?: string, event_id?: string) {
    const params = new URLSearchParams();
    ...
    return handleResponse<{ gallery: any[] }>(res);
  },

  async createGalleryMedia(postData: {
    title: string;
    image_url: string;
  ...
  async likeGallery(id: string, userKey?: string) {
    const res = await fetch(`${API_BASE}/gallery/${id}/like`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ userKey }),
    });
    return handleResponse<{ success: boolean; likes_count: number; is_liked: boolean }>(res);
  },

  async commentGallery(id: string, commentData: { author_name: string; comment: string; author_avatar?: string }) {
    const res = await fetch(`${API_BASE}/gallery/${id}/comment`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(commentData),
    });
    return handleResponse<{ success: boolean; comment: any }>(res);
  },

  async shareGallery(id: string) {
    const res = await fetch(`${API_BASE}/gallery/${id}/share`, {
      method: 'POST',
    });
    return handleResponse<{ success: boolean; shares_count: number }>(res);
  },
