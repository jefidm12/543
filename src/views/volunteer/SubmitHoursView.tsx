...
import { AIHourExtractorCard } from '../../components/AIHourExtractorCard';
import {
  Clock,
  Building,
  Calendar,
...
export const SubmitHoursView: React.FC<SubmitHoursViewProps> = ({
  ...
  // Mode: 'manual' (Direct Form), 'ai' (Gemini AI Extraction), 'presets' (Interactive Presets Assistant)
  const [activeTab, setActiveTab] = useState<'manual' | 'ai' | 'presets'>('manual');
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        {/* Tab Selection */}
        <div className="flex items-center bg-[#07111F] p-1 rounded-2xl border border-white/10 shrink-0 gap-1">
          <button
            type="button"
          ...
          >
            <Layers size={14} />
            <span>Plantillas DMPS</span>
          </button>
        </div>
      </div>

      {/* 1. AI Assistant Extraction Mode */}
      {activeTab === 'ai' && (
        <div className="space-y-6 animate-fadeIn">
          <AIHourExtractorCard
            onApplySingleEntry={(entry) => {
              if (entry.activity_name) setActivityName(entry.activity_name);
              if (entry.organization_name) setOrganizationName(entry.organization_name);
              if (entry.date) setDate(entry.date);
              if (entry.start_time) setStartTime(entry.start_time);
              if (entry.end_time) setEndTime(entry.end_time);
              ...
              if (entry.supervisor_name) {
                if (['Brenda Lucero', 'Carlos Mendoza', 'Maria Santos', 'David Johnson'].includes(entry.supervisor_name)) {
                  setSupervisorSelection(entry.supervisor_name);
                } else {
                  setSupervisorSelection('__CUSTOM__');
                  setCustomSupervisorName(entry.supervisor_name);
                }
              }
              setActiveTab('manual');
            }}
            onBatchSubmitSuccess={async () => {
              await refreshUserData();
              onOpenMyHours();
            }}
          />
      ...
      {/* 2. Activity Presets Assistant Mode */}
      {activeTab === 'presets' && (
        <div className="space-y-6 animate-fadeIn">
