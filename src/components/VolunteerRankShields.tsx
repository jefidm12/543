...
export const VolunteerRankShields: React.FC<VolunteerRankShieldsProps> = ({
  ...
  // Evaluation of 100+ badges catalog with strict school context
  const evaluatedBadges = useMemo(() => {
    const userSchool = volunteerProfile?.school?.trim() || '';
    const statsObj = {
      approvedHours,
      approvedMinutes,
      totalSubmissions,
      ...
      volunteerSchool: userSchool,
      attendedSchools: volunteerProfile?.attended_schools || (userSchool ? [userSchool] : []),
      activityCategories: volunteerProfile?.activity_categories || [],
      activityNames: volunteerProfile?.activity_names || [],
    };
